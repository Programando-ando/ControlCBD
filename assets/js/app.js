...
function clearInputs() {  
  document.getElementById('cate').value='Comida';
...
async function guardar () {
  ...
  datos.append("activi", activi);
  datos.append("cal", cal);
...
Swal.fire({icon: "success", title: "ACTIVIDAD REGISTRADA", text: "LA ACTIVIDAD FUE REGISTRADO CORRECTAMENTE"});

  document.getElementById("cate").value = "";
  document.getElementById("activi").value = "";
  document.getElementById("cal").selectedIndex = 0;
  }else{
...
async function mostrarDatos() {
  ...
  const datos2 = new FormData();
        ...
        let respuesta = await fetch("assets/php/metodos.php", { method: 'POST', body: datos2 });
        let json = await respuesta.json();

        if (json.data.length === 0) {
          document.getElementById('actividad').innerHTML = `<b>Aun no hay actividades o comidas</b>`;
          return;
      }
  ...
  if (datosRecuperados) {
    ...
    json.data.forEach(dato => 
      textoDatos+= `
      <center>
      <div class="card w-75" id="dato-${dato[0]}">
        <div class="card-body">
          Categoría: ${dato[1]}<br>
          Actividad: ${dato[2]}<br>
          Calorías: ${dato[3]}<br>
          <button type="button" class="btn btn-outline-success" onclick="most(${dato[0]})" data-bs-toggle="modal" data-bs-target="#editar"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
  <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
  <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
</svg></button>
          <button type="button" class="btn btn-outline-danger" onclick="borrar(${dato[0]})"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash2" viewBox="0 0 16 16">
